...
def generate_rays(frame_dim, radius, kProbeLoc):
    ...
    return np.array(hit_points), np.array(ray_dirs), np.array(normals)
...
model_file = "NNAttemps/FinalNN/models/final_light_probe_model_NORMALPNG.pth"
output_path = "NNAttemps/FinalNN/output/output00_88.exr"
frame_dim = (1920, 1080)
radius = 0.005
...
def main():
    ...
    # Generate rays
    hit_points, ray_dirs, normals = generate_rays(frame_dim, radius, kprobeloc)
    ...
    input_data = np.concatenate((hit_points_flat, ray_dirs_flat, normals_flat), axis=1)
