...
class CustomDataset(Dataset):
    def __init__(self, folder_path, limit=None):
        ...
        for batch_folder in os.listdir(folder_path):
            ...
            normal_exr_path = get_file_path(batch_folder_path, "hitNormals", ".exr")  # 假设normal文件命名为"normals"
            color_exr_path = get_file_path(batch_folder_path, "ToneMapper", ".exr")
            if hitpos_exr_path and raydir_exr_path and normal_exr_path and color_exr_path:
    ...
    def __getitem__(self, idx):
        ...
        input_data = np.concatenate((hitpos_image.reshape(-1, 3), raydir_image.reshape(-1, 3), normal_image.reshape(-1, 3)), axis=-1)
...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.fc1 = nn.Linear(9, 128)  # 现在输入维度是9（3 for hitpos + 3 for raydir + 3 for normal）
...
def main():
    ...
    model = MLP(input_dim=9, output_dim=3).to(device)  # 输入维度为9
...
final_model_path = "NNAttemps/FinalNN/models/final_light_probe_model_NORMALPNG.pth"
train_path = "C:/Files/CGProject/NNLightProbes/dumped_data/NormalData/train"
val_path = "C:/Files/CGProject/NNLightProbes/dumped_data/NormalData/val"
test_path = "C:/Files/CGProject/NNLightProbes/dumped_data/NormalData/test"
